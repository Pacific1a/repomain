...
io.on('connection', (socket) => {
  ...
  async function spinGlobalGame(game) {
    ...
    // (добавим после определения победителя ниже)
    ...
    for (const player of gameState.players) {
      // Проигравшие = все кроме победителя
  ...
  async function crashCrashGame() {
    ...
    // Проигравшие = те кто не сделал cashout
...
app.post('/api/balance/:telegramId', async (req, res) => {
  try {
    ...
    // ✨ REFERRAL: Отслеживание депозитов (если баланс увеличивается)
    if (finalRubles > 0) {
      try {

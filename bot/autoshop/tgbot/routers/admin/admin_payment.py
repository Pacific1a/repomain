...
from tgbot.keyboards.inline_admin import payment_method_finl, payment_yoomoney_finl, close_finl, payment_qiwi_finl, payment_cactuspay_finl
...
async def payment_methods_edit(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    ...
    elif way_pay == "CactusPay":
        if way_status == "True" and get_payment.cactuspay_token == "None":
            return await call.answer("❗ Добавьте CactusPay кошелёк перед включением Способов пополнений", True)

        Paymentsx.update(way_cactuspay=way_status)
...
# Открытие CactusPay
@router.message(F.text == "🌵 CactusPay")
async def payment_cactuspay_open(message: Message, bot: Bot, state: FSM, arSession: ARS):
    await state.clear()

    await message.answer(
        "<b>🌵 Управление - CactusPay</b>",
        reply_markup=payment_cactuspay_finl(),
    )
...
@router.callback_query(F.data == "payment_cactuspay_edit")
async def payment_cactuspay_edit(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    await state.set_state("here_cactuspay_token")
    await call.message.edit_text(
        "<b>🌵 Введите <code>TOKEN</code> из кабинета CactusPay</b>"
    )

# Принятие токена для CactusPay
@router.message(F.text, StateFilter("here_cactuspay_token"))
async def payment_qiwi_edit_token(message: Message, bot: Bot, state: FSM, arSession: ARS):

    await state.clear()

    Paymentsx.update(
        cactuspay_token=message.text,
    )

    await message.answer(
        "<b>🌵 Token CactusPay - успешно добавлен</b>",

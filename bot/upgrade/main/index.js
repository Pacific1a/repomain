...
function getBalance() {
  const raw = String(balanceMainEl?.textContent || '0').replace(/[^0-9.,-]/g, '').replace(',', '.');
...
// Единая отрисовка UI баланса (новая/старая разметка)
function renderBalanceUI(val){
  const v = Number(val) || 0;
  if (balanceMainEl) balanceMainEl.textContent = v.toFixed(2);
  if (balanceSecondaryEl) balanceSecondaryEl.textContent = String(Math.round(v));
}

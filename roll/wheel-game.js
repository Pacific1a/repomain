(function() {
  ...
  function updateWheel() {
    ...
    segments.forEach((seg, index) => {
      ...
      // Вычисляем центр ЦВЕТНОГО СЕГМЕНТА (не центра круга!)
      // 1. Средний угол между началом и концом сегмента
      const centerAngle = (seg.start + seg.end) / 2;
      
      // 2. Переводим в радианы
      // КРИТИЧНО: conic-gradient(from -90deg) начинается СВЕРХУ (0° = верх)
      // Но Math.cos/sin считают что 0° = ПРАВО
      // Поэтому ОБЯЗАТЕЛЬНО вычитаем 90° чтобы совместить:
      // - Градиент: 0° = верх, 90° = право, 180° = низ, 270° = лево
      // - Math: 0° = право, 90° = низ, 180° = лево, 270° = верх
      ...
      // 4. Радиус - 65% от радиуса колеса (центр цветного сегмента)
      const wheelRadius = 125;
      const radius = wheelRadius * 0.65; // 81.25px - оптимально для центра сегмента
      
      // 5. Вычисляем координаты: аватарка прикрепляется к центру ЦВЕТА
